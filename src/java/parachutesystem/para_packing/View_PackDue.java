...
public class View_PackDue extends AbstractPageBean {
    ...
    private void _init() throws Exception {
        log("<<Entering para_packing View_PackDue>>");
        para_inventory_viewDataProvider.setCachedRowSet((javax.sql.rowset.CachedRowSet) getValue("#{para_packing$View_PackDue.para_inventory_viewRowSet}"));
    ...
    private void changeStatement(int i) {
        try {
            switch (i) {
                ...
                case 1:
                    log("Case 1");
                    para_inventory_viewRowSet.setCommand("SELECT * FROM para_inventory_view WHERE `reserve_chute` = 0 AND `static_line` = 1 AND `lifejacket` = 0 AND `AD` = 0 AND `container` = 0");
                    para_inventory_viewDataProvider.refresh();
                    break;
                case 2:
                    log("Case 2");
                    para_inventory_viewRowSet.setCommand("SELECT * FROM para_inventory_view WHERE `reserve_chute` = 0 AND `static_line` = 0 AND `lifejacket` = 0 AND `AD` = 0 AND `container` = 0");
                    para_inventory_viewDataProvider.refresh();
                    break;
                case 3:
                    log("Case 3");
                    para_inventory_viewRowSet.setCommand("SELECT * FROM para_inventory_view WHERE `reserve_chute` = 1 AND `static_line` = 1 AND `lifejacket` = 0 AND `AD` = 0 AND `container` = 0");
                    para_inventory_viewDataProvider.refresh();
                    break;
                case 4:
                    log("Case 4");
                    para_inventory_viewRowSet.setCommand(
                            "SELECT * FROM para_inventory_view " +
